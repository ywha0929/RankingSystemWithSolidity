...
function signMsg (browser, msg, ) {
  const hash, signature
  browser
    .click('i[id="remixRunSignMsg"]').pause(5000)
    .click('i[id="prompt_text"]')
    .clearValue().setValue(msg)
    .modalFooterOKClick().perform(
      (client, done)=>{
        getText('span[id="remixRunSignMsgHash"]', (v)=>{ hash = v; done()})
       
      }
    )
    .perform(
      (client, done)=>{
        getText('span[id="remixRunSignMsgSignature"]', (v)=>{ signature = v; done()})
      }
    )
    .modalFooterOKClick()
