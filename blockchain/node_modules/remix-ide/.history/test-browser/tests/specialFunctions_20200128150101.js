...
module.exports = {
  ...
  'Use special functions receive/follback - both are declared - fallback should fail cause not payable, sending wei': function (browser) {
    ...
    browser.perform((done) => {
      browser.getAddressAtPosition(0, (address) => {
        ...
        .assert.containsText(`#instance${address} label[id="deployAndRunLLTxError"]`, `the contract should have either 'receive' or payable 'fallback' function`)
        .perform(done)
      })
    })
  },
  ...
  'Use special functions receive/follback - receive is declared, fallback is not declared, sending data': function (browser) {
    browser.waitForElementVisible('#icon-panel', 10000)
  ...
  'Use special functions receive/fallback - receive is not declared, fallback should fail cause not payable, sending wei': function (browser) {
    browser.waitForElementVisible('#icon-panel', 10000)
      .testContracts('fallbackOnly.sol', sources[3]['browser/fallbackOnlyNotPayable.sol'], ['CheckSpecials'])
      .clickLaunchIcon('udapp')
      .selectContract('CheckSpecials')
      .createContract('')
      .clickInstance(3)
  ...
  'Use special functions receive/fallback - both are not declared, sending data and wei': function (browser) {
    browser.waitForElementVisible('#icon-panel', 10000)
      .testContracts('fallbackOnly.sol', sources[4]['browser/fallbackOnlyNotPayable.sol'], ['CheckSpecials'])
      .clickLaunchIcon('udapp')
      .selectContract('CheckSpecials')
      .createContract('')
      .clickInstance(4)
      .perform((done) => {
        browser.getAddressAtPosition(1, (address) => {
          browser.sendLowLevelTx(address, '1', 'oxaa')
          .pause(1000)
          .journalLastChildIncludes('to:CheckSpecials.(fallback)')
          .journalLastChildIncludes('value:1 wei')
          .perform(done)
...
var sources = [
  {
    ...
    'browser/receiveAndFallbackAreNotDeclared.sol': {
      content: `
        contract CheckSpecials {
          function received() payable external {}
        }
      `
    }
  }
