...
// or do we need to only do this in 1 place - I have a feeling that this will happen in multiple places
    ...
    this.contractActionsContainerSingle = yo`
      <div class="${css.contractActionsContainerSingle}" >
        <button
          onclick=${() => { onClick() }}
          class="${css.instanceButton} btn btn-sm"
        >${title}
        </button>
        ${this.basicInputField}
        <i
          class="fas fa-angle-down ${css.methCaret}"
          onclick=${() => { this.switchMethodViewOn() }}
          title=${title}
        >
        </i>
      </div>`
    ...
    var button = yo`<button 
    style="min-width: 43%; text-align: left;"
    onclick=${() => { multiOnClick() }} class="${css.instanceButton}"></button>`
