...
export default class PanelsResize {
  constructor (panel) {
    this.panel = panel
    let self = this;
    let string = panel.style.minWidth
    this.minWidth = string.length > 2 ? string.substring(0, string.length - 2) : 0
  ...
  render () {
    ...
    let moveGhostbar = (event) => { // @NOTE VERTICAL ghostbar
      ...
      if (this.calculatePanelWidth(event) === 0 )
      if(event.x > this.panel.offsetLeft) {
        this.showPanel()
      }
    }
  ...
  setPosition (event) {
    ...
    else if (this.panel.offsetLeft !== 0) this.hidePanel()
  }
