...
class VerticalIconComponent {
  ...
  addIcon ({kind, name, icon, displayName}) {
    let isDark = globalRegistry.get("themeModule").api.currentTheme().quality === 'dark'
    let filter = (isDark) ? "filter: grayscale(1)" : "filter: grayscale(0)"

    this.icons[name] = yo`<div
    class="${css.icon}"
    onclick="${(e) => { this._iconClick(name) }}"
    plugin="${name}"
    title="${displayName || name}" >
      <img src="${icon}" alt="${name}" style="${filter}" />
    </div>`
  ...
  setIconStatus (name, status) {
    ...
    if (status.key) {
      ...
      el.appendChild(yo`<i title="${title}" class="fa fa-filter fa-${key} ${css.status} text-${type}" aria-hidden="true"></i>`)
