...
class VerticalIconComponent {
  ...
  listenOnStatus (api) {
    ...
    const fn = (status) => {
      if (! types.includes(status.type)) throw new Error('key should contain either number or ' + keys.join)
      if (typeof status.key === 'string') {
        if (keys.includes(status.key) || (status.key === '' && status.type === '' && status.title === '')) {
          this.setIconStatus(api.profile.name, status)
        } else throw new Error('key should contain either number or ' + keys.join)
      } else {
