      ...
      const permissionByFunctions = (funcName, fromPlugins) => {
        ...
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const checkbox = fromPluginPermission.allow
            ? yo`<input onchange=${() => updatePermission(fromName)} type="checkbox" checked id="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
            : yo`<input onchange=${() => updatePermission(fromName)} type="checkbox" id="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
          return yo`
            <div class="form-group ${css.permissionKey}">
              ...
              <i onclick="${() => this.clearPersmission(clearPersmission, toPlugin, funcName)}" class="fa fa-trash-alt" data-id="pluginManagerSettingsRemovePermission-${toPlugin}-${funcName}-${toPlugin}"></i>
            </div>
          `
        })
      }
