        ...
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const methodPermission = yo`<div><h5>${fromName}</h5></div>`
            ...
            const checkbox = fromPluginPermission.allow
              ? yo`<input onchange="${() => updatePermission(fromPluginPermission)}" type="checkbox" checked id="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
              : yo`<input onchange="${() => updatePermission(fromPluginPermission)}" type="checkbox" id="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
    
            methodPermission.appendChild(yo`
              <div class="form-group ${css.permissionKey}">
                <div class="${css.checkbox}">
                  ${checkbox}
                  <label for="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" data-id="permission-label-${toPlugin}-${funcName}-${toPlugin}">Allow <u>${fromPluginPermission}</u> to access <u>${funcName}</u></label>
                </div>
                <i onclick="${() => this.clearPersmission(fromPluginPermission, toPlugin)}" class="fa fa-trash-alt" data-id="pluginManagerSettingsRemovePermission-${toPlugin}-${funcName}-${toPlugin}"></i>
              </div>
            `)
