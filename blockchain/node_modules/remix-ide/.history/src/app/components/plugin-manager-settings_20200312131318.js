  ...
  settings () {
    const permissionByToPlugin = (toPlugin, funcObj) => {
      const permissionByMethod = (methodName, fromPlugins) => {
        const togglePermission = (plugin) => {
          this.permissions[toPlugin][methodName][plugin].allow = !this.permissions[toPlugin][methodName][plugin].allow
        }
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const checkbox = fromPluginPermission.allow
            ? yo`<input onchange=${() => togglePermission(fromName)} class="mr-2" type="checkbox" checked id="permission-checkbox-${toPlugin}-${methodName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${methodName}" />`
            : yo`<input onchange=${() => togglePermission(fromName)} class="mr-2" type="checkbox" id="permission-checkbox-${toPlugin}-${methodName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${methodName}" />`
          return yo`
