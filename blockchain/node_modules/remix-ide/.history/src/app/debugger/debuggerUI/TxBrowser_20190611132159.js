...
TxBrowser.prototype.render = function () {
  var self = this
  const txDisabled = this.state.txNumber ? '' : 'disabled'
  var view = yo`<div class="${css.container}">
        <div class="${css.txContainer}">
          ...
          <div class="${css.txbuttons} btn-group p-1">
            <button class='btn btn-primary btn-sm ${css.txbutton}' ${txDisabled} id='load' title='${this.state.debugging ? 'Stop' : 'Start'} debugging' onclick=${function () { self.submit() }}>${this.state.debugging ? 'Stop' : 'Start'} debugging</button>
          </div>
        </div>
        <span id='error'></span>
      </div>`
