...
TxBrowser.prototype.render = function () {
  ...
  let txNumberInput = yo`
    <div style=" position: relative;">
      <input
        ...
        required
        data-require=""
        oninvalid="${this.setCustomValidity('Please provide a valid transaction number, must start with 0x and have length of 22')}"
        pattern="^0[x,X]+[0-9a-fA-F]{22}"
      />
      <div class="invalid-tooltip">
        Please provide a valid transaction number, must start with 0x
      </div>
    </div>
  `
  let txButton = yo`
  ...
  if (!this.state.txNumber) {
    ...
    txNumberInput.removeAttribute('diabled', '')
  }
  if (!this.view) {
    this.view = view
  }
...
TxBrowser.prototype.txInputChanged = function (event) {
  ...
  getElementById('txnumber').setCustomValidity('')
