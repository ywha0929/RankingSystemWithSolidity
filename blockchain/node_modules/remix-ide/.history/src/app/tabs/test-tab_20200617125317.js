...
module.exports = class TestTab extends ViewPlugin {
  constructor (fileManager, filePanel, compileTab, appManager, renderer) {
    ...
    appManager.event.on('ensureActivated', (name) => {
      if (name === 'solidity') this.updateRunAction(true/*fileManager.currentFile()*/)
    })
    appManager.event.on('ensureDeactivated', (name) => {
      if (name === 'solidity') this.updateRunAction(false/*fileManager.currentFile()*/)
    })
  ...
  */
  ...
  updateRunAction (isSolidityActive = false, currentFile) {
    let el = yo`
    ...
    if (/*!currentFile ||*/ !isSolidityActive || !this.listTests().length/* || (currentFile && currentFile.split('.').pop().toLowerCase() !== 'sol')*/) {
      el.setAttribute('disabled', 'disabled')
