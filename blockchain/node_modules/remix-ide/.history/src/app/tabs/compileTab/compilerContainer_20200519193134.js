...
import { canUseWorker, baseURLList, baseURLBin, baseURLWasm } from '../../compiler/compiler-utils'
...
class CompilerContainer {
  ...
  _updateVersionSelector (customUrl = '') {
    ...
    if (customUrl !== '') {
      ...
      url = `${baseURLList[this.data.selectedVersion]}/${this.data.selectedVersion}`
    }
    ...
    minixhr(`${this.data.baseURLBin}/list.json`, (json, event) => {
      ...
      if (event.type !== 'error') {
        try {
          ...
          minixhr(`${this.data.baseURLWasm}/list.json`, (json, event) => {
            ...
            if (event.type !== 'error') {
              try {
                ...
                allVersions.forEach((compiler) => {
                  const rewritten = allVersionsWasm.findIndex(oldCompiler => oldCompiler === compiler)
                  if (-1 !== rewritten) {
                    allVersions[compiler] = allVersionsWasm[rewritten]
                    urls[compiler.path] = this.data.baseURLWasm
                  } else {
                    urls[compiler.path] = this.data.baseURLBin
                  }
                })
                baseURLList = urls
                callback(allVersions, selectedVersion)
              } catch (e) {
                addTooltip('2Cannot load compiler version list. It might have been blocked by an advertisement blocker. Please try deactivating any of them from this page and reload.', e)
              }
            } else {
          ...
          addTooltip('1Cannot load compiler version list. It might have been blocked by an advertisement blocker. Please try deactivating any of them from this page and reload.')
        }
      } else {
