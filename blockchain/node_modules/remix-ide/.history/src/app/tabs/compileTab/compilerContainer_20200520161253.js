...
class CompilerContainer {
  ...
  async fetchAllVersion () {
    ...
    // fetch normal builds
    const {json, event} =  await promisedMiniXhr(`${this.data.baseURLBin}/list.json`)
    // fetch wasm builds
    const {jsonWasm, eventWasm} = await promisedMiniXhr(`${this.data.baseURLWasm}/list.json`)
    if (event.type === 'error' && eventWasm.type === 'error') {
        allVersions = [{ path: 'builtin', longVersion: 'latest local version' }]
        selectedVersion = 'builtin'
        callback(allVersions, selectedVersion)
    }
    try {
      allVersions = JSON.parse(json).builds.slice().reverse()
      selectedVersion = this.data.defaultVersion
      if (this.queryParams.get().version) selectedVersion = this.queryParams.get().version
      if (eventWasm.type !== 'error') {
        allVersionsWasm = JSON.parse(jsonWasm).builds.slice().reverse()
      }
    } catch (e) {
