    ...
    this.runner.run(this.lastCompilationResult, selected, (results) => {
      results.map((result, j) => {
        result.report.map((item, i) => {
          ...
          warningCount++
          const msg = yo`<span><span class="h6">${this.runner.modules()[selected[j]]} </span><span class="font-weight-bold">${result.name}</span> ${location} ${item.warning} ${item.more ? yo`<span><br><a href="${item.more}" target="blank">more</a></span>` : yo`<span></span>`}</span>`
