    ...
    this.runner.run(this.lastCompilationResult, selected, function (results) {
      results.map(function (result, i) {
        result.report.map((item, i) => {
          ...
          warningCount++
          const msg = yo`<span><span class="h6">${this.runner.modules()[i]} </span><span class="font-weight-bold">${result.name}</span> ${location} ${item.warning} ${item.more ? yo`<span><br><a href="${item.more}" target="blank">more</a></span>` : yo`<span></span>`}</span>`
