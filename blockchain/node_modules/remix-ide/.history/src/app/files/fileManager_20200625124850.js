  ...
  async allPaths () {
    ...
    const findPaths = async (path) => {
      ...
      if (this.isDirectory(path)) {
        if (!dirPaths.includes(path)) { dirPaths.push(path); console.log("including ", path) }
        else {console.log("not including ", path)}

        const ls = await this.readdir(path)
        Object.keys(ls).forEach((item, index) => {
         // const pos = item.lastIndexOf('/')
         // const absPath = item
          //if ( pos != -1) item = item.substr(pos + 1, item.length)
    ...
    console.log("laaaaaaaaaaaast ", dirPaths);
    return dirPaths
  }
