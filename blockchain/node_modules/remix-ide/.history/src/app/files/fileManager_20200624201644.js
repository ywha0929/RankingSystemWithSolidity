...
class FileManager extends Plugin {
  ...
  async readdir (path) {
    ...
    return new Promise((resolve, reject) => {
      ...
      provider.resolveDirectory(path, (error, filesProvider) => {
        ...
        console.log("readdit-------path is ", path, " filesProvider is ", filesProvider)
  ...
  async allPaths () {
    const dirPaths = []//['browser', 'browser/test'] // remove this
    // `files` is an array of file paths
    let root = 'browser'

    const findPaths = async (path) => {
      //path = this.removePrefix(path)

      if (this.isDirectory(path)) {
        if (dirPaths.indexOf(path) === -1) dirPaths.push(path)

        const lsik = await this.readdir('browser/tests/ff')
        console.log("<<<<<<<<<<<<<<<<<<<<<< browser/tests/ff'", lsik)
        const ls = await this.readdir(path)
        Object.keys(ls).forEach((item, index) => {
          const curPath = `${path}/${item}`
          console.log( "Object.keys(ls) is ", Object.keys(ls), " ls is ", ls, " path is ", path, " curPath is ", curPath, " item is ",  item, " dirPath is ", dirPaths)
