...
contract Ranking {
    ...
    mapping (bytes32 => uint8) public numSolved;
    // mapping (uint8 => bytes32) public top3;
    constructor(string memory source ) public {
        temp = source;
    }

    function solvedProblem(string memory user_name) public {
    ...
    function numofSolved(string memory User_name_string) public returns (uint8)
    {
        bytes32 User_name_bytes = _stringToBytes32(User_name_string);
        return (numSolved[User_name_bytes]);
    }
    ...
    function getRankByName(string memory user_name, uint mode) public returns (uint)
    {
        bytes32 User_name = _stringToBytes32(user_name);
