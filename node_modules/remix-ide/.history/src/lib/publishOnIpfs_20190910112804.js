...
async function initIpfs () {
  let node = await IPFS.create({ start: false })
...
async function ipfsVerifiedPublish (content, expectedHash, cb) {
  ...
  if (err) {
    cb(err)
  } else if (results[0].hash !== expectedHash) {
    cb(null, { message: 'hash mismatch between solidity bytecode and uploaded content.', url: 'dweb:/ipfs/(.+)' + ret, hash: ret })
  } else {
    cb(null, { message: 'ok', url: 'dweb:/ipfs/(.+)' + ret, hash: ret })
  }
  closeIpfs(ipfs)
}
