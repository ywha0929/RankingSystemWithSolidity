...
class CompilerContainer {
  ...
  fetchAllVersion (callback) {
    ...
    // fetch normal builds
    await promisedMiniXhr(`${this.data.baseURLBin}/list.json`, (json, event) => {
    ...
    await promisedMiniXhr(`${this.data.baseURLWasm}/list.json`, (json, event) => {
      // @TODO: optimise and cache results to improve app loading times #2461
      if (event.type !== 'error') {
        try {
          const data = JSON.parse(json)
    ...
    console.log(Object.keys(pathToURL).length)
    callback(allVersions, selectedVersion)
