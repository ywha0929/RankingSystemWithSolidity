...
class TestTabLogic {
  ...
  generateTestContractSample (hasCurrent, fileToImport, contractName = 'testSuite') {
    const parsePath = window.remixFileSystem.parse(fileToImport)
    const relative = window.remixFileSystem.relative(this.currentPath, parsePath.dir)
    const comment = hasCurrent ? `import "${relative}/${parsePath.base}";` : '// Import here the file to test.'
