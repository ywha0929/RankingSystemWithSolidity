...
class TestTabLogic {
  ...
  allPaths () {
    this.dirPaths = ['browser', 'browser/test'] // remove this
    ...
    let findPaths = (path) => {
      ...
      if (window.remixFileSystem.statSync(path).isDirectory()) {
        if (this.dirPaths.indexOf(path) === -1) this.dirPaths.push(path)

        const ls = window.remixFileSystem.readdirSync(path)
        if (ls.length !== 0) {
          ls.forEach((dir, index) => {
            const curPath = `${path}/${dir}`
            if (window.remixFileSystem.statSync(curPath).isDirectory()) {
              this.findPaths(curPath)
            }
          })
        }
      }
