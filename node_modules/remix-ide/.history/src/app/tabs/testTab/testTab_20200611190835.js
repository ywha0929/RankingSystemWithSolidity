...
class TestTabLogic {
  ...
  generateTestFile () {
    ...
    if (!fileName) {
      fileName = this.currentPath + '/newFile.sol'
    }
    const fileProvider = this.fileManager.fileProviderOf(this.currentPath)
    if (!fileProvider) return
    const splittedFileName = fileName.split('/')
    let fileNameToImport = (!fileName) ? fileName : splittedFileName[splittedFileName.length - 1]
    helper.createNonClashingNameWithPrefix(this.currentPath + '/' + fileNameToImport, fileProvider, '_test', (error, newFile) => {
