...
staticAnalysisView.prototype.run = function () {
  ...
  if (this.lastCompilationResult && selected.length) {
    ...
    const groupedModules = utils.groupBy(preProcessModules(this.runner.modules()), 'categoryId')
      ...
      let alreadyExistedEl = warningContainer.querySelector('[id="staticAnalysisModule', moduleName, '"]')
      if (! alreadyExistedEl) {
        alreadyExistedEl.appendChild(yo`
      ...
      results.map((result, j) => {
        result.report.map((item, i) => {
          ...
          self._components.renderer.error(msg, alreadyExistedEl, {type: 'staticAnalysisWarning alert alert-warning', useSpan: true})
        })
      })
