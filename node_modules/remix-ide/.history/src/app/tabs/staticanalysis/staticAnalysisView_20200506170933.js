  ...
  if (this.lastCompilationResult && selected.length) {
    ...
    this.runner.run(this.lastCompilationResult, selected, (results) => {
      ...
      results.map((result, j) => {
        ...
        let alreadyExistedEl = this.viewquerySelector(`[id="staticAnalysisModule${moduleName}"]`)
        if (! alreadyExistedEl) {
          warningContainer.append(`
            <div class="card mb-3" name="staticAnalysisModules" id="staticAnalysisModule${moduleName}">
              <span class="card-header">${moduleName}</span>
            </div>
          `)
        }
        ...
        result.report.map((item, i) => {
          ...
          if (item.location) {
            ...
            location = (location.start.line + 1) + ':' + (location.start.column + 1) + ':'
          } else {
          ...
          const msg = yo`
            <span class="d-flex flex-column">
              <span class="text-dark h6">${result.name}</span>
              ${item.warning}
              ${item.more ? yo`<span><a href="${item.more}" target="blank">more</a></span>` : yo`<span></span>`}
              <span class="text-dark" title="Position in ${Object.keys(self.lastCompilationResult.contracts)[file]}">Pos: ${location}</span>
            </span>`
          self._components.renderer.error(
            msg,
            this.viewquerySelector(`[id="staticAnalysisModule${moduleName}"]`),
            {type: 'staticAnalysisWarning mx-2 alert alert-warning', useSpan: true}
          )
        })
      })
      // hide empty cards for modules
      this.view.querySelectorAll('[name="staticAnalysisModules"]').forEach ((card) => {
        if (!card.getElementsByClassName('alert-warning').length) card.hidden = true
      })
