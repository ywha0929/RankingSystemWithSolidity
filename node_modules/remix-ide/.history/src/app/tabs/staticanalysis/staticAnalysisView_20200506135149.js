    ...
    const groupedModules = utils.groupBy(preProcessModules(this.runner.modules()), 'categoryId')
      results.map((result, j) => {
        ...
        result.report.map((item, i) => {
          ...
          self._components.renderer.error(msg, document.querySelector(`[id="staticAnalysisModule${moduleName}"]`), {type: 'staticAnalysisWarning mx-2 alert alert-warning d-flex flex-column', useSpan: true})
        })
        // hide empty cards for modules
        this.view.querySelectorAll('[name="staticAnalysisModules"]').forEach ((cart) => {
          if (!cart.getElementsByClassName('alert-warning').length) cart.hidden = true
        })
      })
