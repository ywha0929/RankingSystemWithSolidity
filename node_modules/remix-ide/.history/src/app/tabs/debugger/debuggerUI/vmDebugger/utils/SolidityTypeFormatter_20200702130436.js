...
function formatSelf (key, data) {
  ...
  return yo `
    <label class="mb-0 d-flex flex-column" style='${keyStyle};white-space:pre-wrap;'>
      <span class="mb-0 d-flex font-weight-bold" style="min-width: 15%">
        <label>${' ' + key}:</label>
        <label class="mb-0" style=${style}>${' ' + data.self}</label>
      </span>
      <span class="mb-0 d-flex font-weight-bold">
        <label style='font-style:italic'>${data.isProperty || !data.type ? '' : ' ' + data.type}</label>
      </span>
    </label>`
}
