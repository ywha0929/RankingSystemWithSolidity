...
function staticAnalysisView (localRegistry) {
  ...
  this.compilationTarget = this._renderTarget('')
  // dependencies
  self._deps = {
  ...
  self._deps.pluginManager.event.register('sendCompilationResult', (file, source, languageVersion, data) => {
    ...
    if (self.view.querySelector('#autorunstaticanalysis').checked) {
      yo.update(this.compilationTarget, this._renderTarget(source.target))
      self.run()
    }
  })
}

staticAnalysisView.prototype._renderTarget = function (file) {
  return  yo`
  <h6 class="bg-light input-group mb-2">
    Results for 
    <label id="resultFile" class="font-italic border-0 px-2 text-dark">${file}</label>
  </h6>`
}
  ...
  var view = yo`
    <div class="${css.analysis}">
      <div class="${css.buttons}">
        ...
        <div class="${css.resultTitle} mx-2">${this.compilationTarget}</div>
