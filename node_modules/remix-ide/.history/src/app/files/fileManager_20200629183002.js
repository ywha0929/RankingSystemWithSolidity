...
class FileManager extends Plugin {
  ...
  // returns the list of directories of path directory
  dirList (path) {
    const dirPaths = []
        ...
        if (this.isDirectory(path)) {
          this.readdir(path).then((ls) => {
            const promises = Object.keys(ls).map((item, index) => {
              ...
              console.log('current path is ', curPath)
              if (ls[item].isDirectory && !dirPaths.includes(path)) {
                  dirPaths.push(path)
    ...
    return new Promise ((resolve, reject) => { collectList(path) })
  }
  ...
  allPaths () {
    ...
    if (this.appManager.isActive('remixd'))
      roots.push(findPaths('localhost'))
