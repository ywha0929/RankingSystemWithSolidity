    ...
    if (this.locahostProvider.isConnected()) {
      ...
      modalDialog(
        ...
        { label: 'Connect',
          fn: () => {
            try {
              console.log('connecting activate')
              super.activate(()=>{ connection(error) }).then(() => {console.log('then err')}).catch((e) => {console.log('catching err')})
              super.socket.addEventListener('error', async (event) => {
                console.log('WebSocket error remixd: ', event)
              })
