...
class VerticalIconComponent {
  ...
  select (nameToActivate) {
    ...
    if (currentActive) currentActive.classList.remove(css.active)
    // Step 2 : Add active for the new activated icon
    const nextActive = this.view.querySelector(`.icon[plugin=${nameToActivate}`)
    if (nextActive) nextActive.classList.add(css.active)
    // Step 3 : Set invert color depending of the current theme
    const themeType = globalRegistry.get("themeModule").api.currentTheme().quality
    const invert = themeType === 'dark' ? 1 : 0
    currentActive.style.setProperty('filter', 'invert(0.5)')
    nextActive.style.setProperty('filter', `invert(${invert})`)
