...
class VerticalIconComponent {
  ...
  listenOnStatus (api) {
    if (!api.events) return
    const fn = (status) => {
      if (status.type == 'warning' || status.type == 'danger' || status.type == 'code' || status.type == 'check')
  ...
  setIconStatus (name, status) {
    ...
    if (status.key) {
      ...
      switch(type) {
        case 'code':
        classes += ' fas fa-asterisk text-info'
        case 'check':
      ...
      el.appendChild(yo`<span 
        title="${title}"
        class="${type == 'code' ? 'fas fa-asterisk text-info' +  css.status : 'badge badge-pill badge-' + type + " " + css.status}"
        aria-hidden="true"
      >
      ${type == 'success' ? 'v' : status.key}
      </span>`)
