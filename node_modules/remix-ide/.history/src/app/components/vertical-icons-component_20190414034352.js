...
class VerticalIconComponent {
   ...
   */
  removeActive () {
    const currentActives = this.view.querySelectorAll(`.image`)
    currentActives.forEach(function(im){image.style.setProperty('filter', 'invert(0.5)')})
    const currentActive = this.view.querySelectorAll(`.${css.active}`)
    if (currentActive) {
