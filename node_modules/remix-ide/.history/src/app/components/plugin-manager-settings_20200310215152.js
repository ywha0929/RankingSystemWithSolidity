        ...
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const methodPermission = yo`<div><h5>${fromName}</h5></div>`
            ...
            const checkbox = fromPluginPermission.allow
              ? yo`<input onchange="${() => updatePermission(fromPluginPermission)}" type="checkbox" checked id="permission-${name}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
