    ...
    const permissionByToPlugin = (toPlugin, funcObj) => {
      const permissionByMethod = (methodName, fromPlugins) => {
        function updatePermission (plugin) {
          self.permissions[toPlugin][methodName][plugin].allow = !self.permissions[toPlugin][methodName][plugin].allow
        }
        let self = this
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const checkbox = fromPluginPermission.allow
            ? yo`<input onchange=${() => updatePermission(fromName)} class="mr-2" type="checkbox" checked id="permission-checkbox-${toPlugin}-${methodName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${methodName}" />`
            : yo`<input onchange=${() => updatePermission(fromName)} class="mr-2" type="checkbox" id="permission-checkbox-${toPlugin}-${methodName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${methodName}" />`
          return yo`
