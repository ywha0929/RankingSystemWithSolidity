...
class VerticalIconComponent {
  ...
  listenOnStatus (api) {
    ...
    const fn = (status) => {
      if (!types.includes(status.type)) throw new Error(`type should be ${keys.join()}`)
      if (typeof status.key === 'string') {
        if (! keys.includes(status.key) && !(status.key === '')) {
          throw new Error('key should contain either number or ' + keys.join())
        } else {
          // set default value for type
          if (status.type === '') status.type = 'info'
          if (status.key === 'none') {
            this.removeIconStatus(api.profile.name)
          } else this.setIconStatus(api.profile.name, status)
        }
      } else {
        // key contains number of errors or warnings
        if (status.type === 'error') status.type = 'danger' // to use with bootstrap
        status.key = status.key.toString()
  ...
  /**
   * Remove status for the @arg name
   * @param {String} name
   * @param {Object} status
   */
  removeIconStatus(name) {
    ...
    if (status.key) {
      ...
      switch (key) {
        ...
        case 'edited':
          ...
          break
        case 'spinner':
          classes += ' fas fa-spinner text-' + type
          break
        case 'fail':
          classes += ' fas fa-exclamation-triangle text-' + type
          break
        default: {
