...
class VerticalIconComponent {
  ...
  render () {
    let homeProfile = this.store.getOne('home')
    let home = yo`
    <div
      class="${css.icon}"
      onclick="${(e) => {
        this._iconClick(name)
        this.globalRegistry.get('appManager').api.ensureActivated('home')
      }}"
      plugin="${homeProfile.name}" title="${homeProfile.displayName}"
    >
      <img class="image" src="${homeProfile.icon}" alt="${homeProfile.name}"/>
    </div>`
