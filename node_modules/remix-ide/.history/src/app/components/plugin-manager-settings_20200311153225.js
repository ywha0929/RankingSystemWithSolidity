    ...
    const permissionByToPlugin = (toPlugin, funcObj) => {
      const permissionByFunctions = (funcName, fromPlugins) => {
        function updatePermission (plugin) {
          this.permissionsplugin.allow = !plugin.allow
        }
        ...
        return Object.keys(fromPlugins).map(fromName => {
          ...
          const checkbox = fromPluginPermission.allow
            ? yo`<input onchange="${() => updatePermission(fromPluginPermission).bind(this)}" type="checkbox" checked id="permission-checkbox-${toPlugin}-${funcName}-${toPlugin}" aria-describedby="module ${fromPluginPermission} ask permission for ${funcName}" />`
