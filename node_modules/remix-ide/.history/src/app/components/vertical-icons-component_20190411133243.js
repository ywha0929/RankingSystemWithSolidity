...
class VerticalIconComponent {
  ...
  {
    if (!type) {
      type = globalRegistry.get("themeModule").api.currentTheme().quality
    }

    for (let key in this.icons) {
      if (type === 'dark') {
        this.iconKind[key].className =  this.iconKind[key].className.replace(/\light\b/g, "");
        this.iconKind[key].classList.add(`${css.dark}`)
      } else {
        //this.iconKind[key].classList.remove(`${css.dark}`)
        this.iconKind[key].className =  this.iconKind[key].className.replace(/\dark\b/g, "");
        this.iconKind[key].classList.add(`${css.light}`)
      }
    }
  }
  /**
  ...
  select (name) {
    ...
    let currentActive = this.view.querySelector(`.active${themeType}`)
    let activeType = `.active${themeType}`
    
    if (currentActive) {
      let currentTitle = currentActive.getAttribute('title')
      currentActive.classList.toggle(activeType)
      if (currentTitle !== name) {
        let activate = this.view.querySelector(`[plugin="${name}"]`)
        if (activate) activate.classList.toggle(activeType)
      }
    } else {
      let activate = this.view.querySelector(`[plugin="${name}"]`)
      if (activate) activate.classList.toggle(activeType)
