...
class VerticalIconComponent {
  ...
  select (name) {
    ...
    let activeType = globalRegistry.get("themeModule").api.currentTheme().quality === 'light' ?
      `${css.darkActive}` : `${css.lightActive}` 
    if (currentActive) {
      let currentTitle = currentActive.getAttribute('title')
      currentActive.classList.toggle(`${css.lightActive}`)
      if (currentTitle !== name) {
        let activate = this.view.querySelector(`[plugin="${name}"]`)
        if (activate) activate.classList.toggle(`${css.lightActive}`)
      }
    } else {
      let activate = this.view.querySelector(`[plugin="${name}"]`)
      if (activate) activate.classList.toggle(`${css.lightActive}`)
