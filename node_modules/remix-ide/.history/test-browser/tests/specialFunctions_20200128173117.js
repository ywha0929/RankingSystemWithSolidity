...
module.exports = {
  ...
  'Use special functions receive/fallback - both are not declared, sending data and wei, should fail': function (browser) {
    browser.waitForElementVisible('#icon-panel', 10000)
    .testContracts('receiveAndFallbackAreNotDeclared.sol', sources[4]['browser/receiveAndFallbackAreNotDeclared.sol'], ['CheckSpecials'])
    .clickLaunchIcon('udapp')
    .selectContract('CheckSpecials')
    .createContract('')
    .clickInstance(4)
    ...
    .perform((done) => {
      browser.getAddressAtPosition(4, (address) => {
        browser.sendLowLevelTx(address, '1', '0xaa')
        .pause(1000)
        .journalLastChildIncludes('The called function should be payable if you send value and the value you send should be less than your current balance.')
        .perform(done)
      })
    })
    .end()
...
var sources = [
  ...
  },
  {
    'browser/receiveAndFallbackAreNotDeclared.sol': {
      content: `
        contract CheckSpecials {
          function myFunction() payable external {}
        }
      `
    }
  },
