...
function testSignature (browser, callback) {
  let hash, signature
  browser.clickLaunchIcon("run").perform((client, done)=>{
    contractHelper.signMsg("test message", (h, s)=>{
      hash = h
      signature = s
      contractHelper.addFile(browser, "browser/signMassage.sol", sources[6]["browser/signMassage.sol"], ()=>{
        contractHelper.selectContract(browser, 'SignMassageTest', () => { // deploy lib
          contractHelper.createContract(browser, '', () => {
            // deploy

            browser.clickFunction("testRecovery - call", {types: "bytes32 hash, bytes sig", values: `"${hash}","${signature}"`}).perform(
              ()=>{
                contractHelper.verifyCallReturnValue(browser, "", ['0: address: 0xCA35b7d915458EF540aDe6068dFe2F44E8fa733c'] )
              }
            )
          })
        })
      })
      browser.clickLaunchIcon("solidity").addFile()
    })
  })
}
/*
