...
module.exports = {
  ...
  'Use special functions receive/fallback - receive and fallback are not declared, sending nothing': function (browser) {
    browser.waitForElementVisible('#icon-panel', 10000)
    .testContracts('receiveAndFallbackBothPayable.sol', sources[5]['browser/receiveAndFallbackBothPayable.sol'], ['CheckSpecials'])
    .clickLaunchIcon('udapp')
    .selectContract('CheckSpecials')
    .waitForElementVisible('#value')
    .clearValue('#value')
    ...
    .perform((done) => {
      browser.getAddressAtPosition(5, (address) => {
        ...
        .pause(1000)
        .waitForElementVisible(`#instance${address} label[id="deployAndRunLLTxError"]`)
        .assert.containsText(`#instance${address} label[id="deployAndRunLLTxError"]`, `Both 'receive' and 'fallback' function are not defined`)
        .perform(done)
      })
    })
    .end()
