...
function signMsg (browser, msg, ) {
  ...
  browser.click('i[id="remixRunSignMsg"]').pause(5000).click('i[id="prompt_text"]').setValue(msg).modalFooterOKClick().perform(
    ()=>{
      getText('span[id="remixRunSignMsgHash"]', (v)=>{hash = v; done()})
    }
  ).perform(
    ()=>{
      getText('span[id="remixRunSignMsgSignature"]', (v)=>{hash = v; done()})
    }
  )
  return
}
